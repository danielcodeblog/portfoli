...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#0F211D]/60 backdrop-blur-md flex justify-end">
      <div className="w-full lg:w-[500px] bg-white h-full flex flex-col border-l border-[#0F211D]/10 shadow-2xl relative animate-in slide-in-from-right duration-500">
        <div className="p-8 border-b border-[#0F211D]/10 flex justify-between items-center bg-white">
          <div>
          ...
          {messages.map((msg, idx) => (
            <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              <div className={`max-w-[85%] p-6 rounded-3xl text-sm leading-relaxed font-bold ${
                msg.role === 'user' 
                  ? 'bg-[#0F211D] text-white rounded-tr-none' 
        ...
        <div className="p-8 bg-white border-t border-[#0F211D]/5">
          <form onSubmit={handleSubmit} className="relative">
            <input
              ...
              className="w-full bg-white border-2 border-[#0F211D]/5 rounded-2xl p-6 pr-16 text-sm font-bold focus:outline-none focus:border-[#D4FF3F] transition-all text-[#0F211D] placeholder:text-[#0F211D]/20"
            />
            <button 
              type="submit"
